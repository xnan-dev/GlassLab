...
// PUBLIC UTILITIES
//***************************************************************
function findGm1InstrumentPatchNumber($instrument_name) {
	$map=$this->getGm1InstrumentPatchMap();
	$reverse_map=array_flip($map);
	if (!array_key_exists($instrument_name, $reverse_map)) $this->_err("unknown instrument name:$instrument_name");
	$number=$reverse_map[$instrument_name];	
	return $number;
}

function getGm1InstrumentPatchMap() {
	return array(
